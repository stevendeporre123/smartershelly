...
function renderWifiStatusIcon(ssid) {
  const hasSsid = Boolean(ssid);
  const colorClass = hasSsid ? 'text-emerald-500' : 'text-slate-400';
  const icon = hasSsid ? 'wifi' : 'wifi-off';
  const label = hasSsid ? `Connected to ${ssid}` : 'No Wi-Fi information available';
  return `
    <span class="inline-flex items-center" title="${escapeHtml(label)}">
      <span data-feather="${icon}" class="h-4 w-4 ${colorClass}"></span>
...
function renderDevicesTable(container) {
  ...
  if (!filteredDevices.length) {
    container.innerHTML = `
      <div class="rounded-xl border border-dashed border-slate-300 bg-slate-50/70 px-4 py-8 text-center">
        <p class="text-sm text-slate-500">No devices found${query ? ' for this search.' : '.'}</p>
      </div>
    `;
    return;
  }
  ...
  const rows = sortedDevices
    .map((device) => {
      ...
      const isOffline =
        device.status === 'offline' || device.diffStatus === 'offline' || device.isOnline === false;
      ...
      return `
        <tr class="hover:bg-slate-50 ${rowClass}">
          ...
          <td class="px-4 py-3">${renderWifiStatusIcon(device.wifiSsid)}</td>
          ...
          <td class="px-4 py-3">
            <div class="flex items-center justify-end gap-2">
              <button
                type="button"
                class="rounded-lg border border-slate-200 bg-white p-2 text-slate-500 shadow-sm transition hover:bg-slate-100 hover:text-slate-700"
